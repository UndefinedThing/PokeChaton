...
const Speace &PokeChat::getSpeaces() const {
    return speaces;
}

void PokeChat::setSpeaces(const Speace &speaces) {
    PokeChat::speaces = speaces;
}

PokeChat::PokeChat(int idPokeChat, const string &name, const Gender &gender, const vector<Type> &type, int lvl,
                   int health, int attaqueModifer, int defenceModifer, int spécialModifer, int vitesseModifer,
                   int precisinModifer, const Speace &speaces1, const vector<Attack> &atkList) : IdPokeChat(idPokeChat),
                   ...
                   precisinModifer(precisinModifer),speaces1(speaces1),AtkList(atkList) {}
